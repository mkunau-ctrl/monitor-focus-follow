...
| `src/Native.cs` | Alle echten Windows-API-Aufrufe (C#, wird beim Start per `Add-Type` kompiliert): Mausposition, Fenster unter dem Zeiger, Fenster aktivieren (inkl. Umgehung der Vordergrund-Sperre), Maustasten-/Tastenzustand, Vollbild-Erkennung, DPI-Awareness, eigenes Konsolenfenster verstecken (`HideConsoleWindow`). Enthält keine Entscheidungslogik. |
...
| Datei | Zuständig für |
|---|---|
| `Install-Autostart.ps1` | Legt die geplante Aufgabe `MonitorFocusFollow` an (Auslöser „bei Anmeldung", 15 s Verzögerung, versteckt, Neustart nach Absturz). Kein Admin/Passwort nötig. Räumt eine alte Autostart-Verknüpfung samt Registry-Eintrag weg und startet das Programm sofort mit. |
| `Uninstall-Autostart.ps1` | Entfernt die geplante Aufgabe wieder (und eine evtl. noch vorhandene alte Verknüpfung). Beendet keinen laufenden Prozess. |
...
| `usb/Setup.cmd` + `usb/Setup.ps1` | **Variante A:** kopiert das Programm vom Stick nach `%LOCALAPPDATA%\monitor-focus-follow`, ruft dann `Install-Autostart.ps1` auf (geplante Aufgabe + Sofortstart). `.cmd` = Doppelklick-Starter, `.ps1` = die eigentliche Arbeit. Kein Admin nötig. |
| `usb/Start-Portabel.cmd` | **Variante B:** startet das Programm direkt vom Stick, ohne Installation. Läuft, solange der Stick steckt. |
| `usb/Deinstallieren.cmd` + `usb/Deinstallieren.ps1` | Beendet die laufende Instanz, entfernt die geplante Aufgabe (und eine evtl. alte Verknüpfung) und den Programmordner vom PC. |
